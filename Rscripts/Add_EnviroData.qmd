...
df.aggEARLY = subset(df.agg, HAUL_YEAR %in% 1990:2011 | HAUL_MONTH < 05 & HAUL_YEAR == 2012)
...
```
2018-present (Sea Surface Temperature, NOAA geopolar blended - Monthly, 2002-Present (2017
Reanalysis))
```{r, eval = F}
p <- proc.time() ## set timer
...
df.aggLATE = subset(df.agg, HAUL_YEAR %in% 2019:2024 |
...
list.files(dump.file, pattern = "*sst.csv", full.names = TRUE) %>%
  ...
  purrr::map_dfr(., readr::read_csv, col_names=FALSE) %>%
  #lapply(FUN = read.csv, skip=2, header = TRUE, na.strings = 'NaN') %>%
  ## bind them together
  #bind_rows %>%
  ## skip NAs
  #na.omit %>%
...
for(u in 1:nrow(utp)){
    ...
    D = geosphere::distm(subDATA[,c('LON2','LATITUDE')], subSST[,c('LON','LAT')])
...
and saved as CSV
```{r}
pdoi = read.csv('G:/Enviro/PDOindex_08-25.csv', header = T)
...
df_pdoi<-merge(df,pdoi,by.x = c('HAUL_YEAR','HAUL_MONTH'), by.y = c('YEAR', 'MONTH'))
...
```{r}
soi = read.table('G:/Enviro/SOI_08-25.txt', header = T,sep=",")
...
df_soi<-merge(df_pdoi,soi,by.x = c('HAUL_YEAR','HAUL_MONTH'), by.y = c('YEAR', 'MONTH'))
...
nino = read.csv('G:/Enviro/nino34_08-25.csv')
df_sst_pdoi_soi_oni = merge(df_sst_pdoi_soi,nino, by.x = c('HAUL_YEAR','HAUL_MONTH'), by.y = c('YR', 'MON'))
df_oni = merge(df_soi,nino, by.x = c('HAUL_YEAR','HAUL_MONTH'), by.y = c('YR', 'MON'))
...
write.csv(df_oni,file='HI CPUE/BUM_CPUEnoSST.csv',row.names  = F)

write.csv(df_sst_pdoi_soi_oni,file='HI CPUE/BUM_CPUEwSST.csv',row.names  = F)
```
...
Need to find a new source for MLD. I can't remember which model I pulled from last time, but the new downscaled data from CEFI for our region might be a good source once its available.
```{r, fig.height = 5, fig.width = 7}
