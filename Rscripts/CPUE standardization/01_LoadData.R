...
#  HI_stats <- dbGetQuery(con, paste("SELECT * FROM LLDS.LLDS_RPT_STATS_HC20250315_RFMO", " WHERE  CONF !='0'", sep = ""))
...
# HI_all_gears <- dbGetQuery(con, paste("SELECT * FROM LLDS.HM_HICA_SPECIES_20250315", sep = ""))
...
#  AS_stats <- dbGetQuery(con, paste("SELECT * FROM LLDS.LLDS_RPT_STATS_AS20250315_RFMO"," WHERE CONF !='0'", sep = ""))
# 
...
# save(HI_stats, HI_all_gears, AS_stats, file="BUM_CatchData.Rdata")

## load the previously saved .Rdata file
load("C:/Users/Michelle.Sculley/Documents/2025_PacificBUM/BUM_CatchData.Rdata") # Don't load .Rdata on to github.
