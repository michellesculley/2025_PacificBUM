...
chunk_size <- 10000  # Number of rows to pull in each chunk
offset <- 0
...
## reshape the data into a wide dataframe, and merge the lists
reshape_data <- function(df) {

  # Apply the pivot_wider transformation
  reshaped_df <- df %>%
    pivot_wider(
      id_cols = c(SET_YEAR, SET_MONTH, SET_DAY,VESSEL_NAME,PERMIT_NUMBER, COMMERCIAL_MARINE_LICENSE_NUM,SERIAL_NUMBER2,BEGIN_SET_TIME),
      names_from = SPECIES_CODE,
      values_from = c(NUMBER_OF_SHARKS_FINNED,NUMBER_OF_FISH_KEPT,NUMBER_OF_FISH_RELEASED,NUMBER_RELEASED_ALIVE_UNINJURE,NUMBER_RELEASED_INJURED,NUMBER_RELEASED_DEAD),
      names_sep = "_",
      values_fill=0
    )
...
# 2. Use lapply() to apply the function to each data frame in your list
# The code assumes your structure is list_of_lists -> list -> dataframe
# If you have a single list of dataframes, you can skip the second lapply()

logbook_list <- lapply(data_chunks, function(inner_list) {
  lapply(inner_list, reshape_data)
})
